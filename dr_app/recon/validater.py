...
class DRBatch:
    ...
    def count(self, column: str, group_by_column_names: list[str]):
        ...
        if isinstance(self.df_src, pd.DataFrame):
            if column in self.df_src.columns:
                if check_if_columns_in_df(df=self.df_src, columns=group_by_column_names):
        ...
        expected_value = {
            ...
            "measure_value": self.df_recon[mask]["measure_value"].apply(pd.to_numeric, errors = 'coerce').to_list()
        }
        actual_value = {
    ...
    def distinct(self, column: str, group_by_column_names: list[str]):
        ...
        if isinstance(self.df_src, pd.DataFrame):
            if column in self.df_src.columns:
                if check_if_columns_in_df(df=self.df_src, columns=group_by_column_names):
                    ds = self.df_src.groupby(group_by_column_names)[column].nunique(dropna=False)
                else:
                    # Group by all records if the given column is not in the dataframe
    ...
    def sum(self, column: str, group_by_column_names: list[str]):
        ...
        if isinstance(self.df_src, pd.DataFrame):
            if column in self.df_src.columns:
                if check_if_columns_in_df(df=self.df_src, columns=group_by_column_names):
                    ...
                    ds = self.df_src.groupby(group_by_column_names)[column].apply(lambda x : x.astype(float).sum())
                else:
                    # Group by all records if the given column is not in the dataframe
        ...
        actual_value = {
            ...
            "measure_value": ds.fillna(0).apply(pd.to_numeric, errors = 'coerce').tolist()
        }
